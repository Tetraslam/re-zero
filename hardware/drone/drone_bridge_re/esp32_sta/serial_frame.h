...
static inline bool sf_send(Stream &s, uint8_t type, uint16_t conn, uint16_t port, const uint8_t *payload, uint16_t paylen) {
  const uint16_t hdr_len = 8;                  // ver..paylen
  ...
  uint16_t crc = 0xFFFF;
  crc = sf_crc16_update(crc, &pre[4], hdr_len);
  if (paylen && payload) crc = sf_crc16_update(crc, payload, paylen);

  s.write(pre, sizeof(pre));
  if (paylen && payload) s.write(payload, paylen);
  uint8_t crc_le[2];
  sf_write_u16(crc_le, crc);
  s.write(crc_le, 2);
  return true;
}
 ...
 public:
  ...
  bool poll(Stream &s, SfHeader &out_h, uint8_t *out_payload, uint16_t out_payload_cap, uint16_t &out_payload_len) {
    while (s.available()) {
      ...
      if (inner_len < 10 || inner_len > MAX_FRAME) {
        _buf_len = 0;
        continue;
      }
      ...
      if (h.ver != SF_VER || (uint32_t)h.paylen + 8 + 2 != inner_len) {
        _buf_len = 0;
        continue;
      }
      ...
      uint16_t crc = 0xFFFF;
      crc = sf_crc16_update(crc, p, 8);
      crc = sf_crc16_update(crc, payload, h.paylen);
      if (crc != want_crc) {
        memmove(_buf, _buf + 1, _buf_len - 1);
        _buf_len--;
        continue;
